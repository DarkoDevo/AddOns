local MODULE_MAJOR, MINOR = "LibGCDCPSettingsMode-1.0", 16011001
...
local State = {
	...
	newTagResolvers = {},
	prefix = "LibGCDCPSettings_",
	prefixSet = false,
	prefixWarned = false,
	rootName = "LibGCDCP Settings",
	categories = {},
	elements = {},
	categoryCacheByID = {},
	categoryCacheByName = {},
...
local function shouldShowNewTag(category)
	...
	local tag = category._LibGCDCPNewTagID or State.categoryTags[category:GetID()]
...
local function registerCategory(name, parent, sort, newTagID, prefixOverride)
	...
	if parent == nil then
		...
		cat._LibGCDCPNewTagID = newTagID
...
local function registerSetting(cat, key, varType, name, default, getter, setter, data)
	...
	if not (State.prefixSet or prefix or variable) then
		error(
			("LibGCDCPSettingsMode: SetVariablePrefix(...) must be called before registering settings (missing before key '%s'). Alternatively pass data.prefix or data.variable."):format(
				tostring(key)
			)
		)
	end
...
function lib:CreateInput(cat, data)
	...
	local initializer = Settings.CreateControlInitializer(
		"LibGCDCPf093333_InputControlTemplate",
		setting,
		nil,
		data.desc
	)
...
function lib:CreateScrollDropdown(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibGCDCPf093333_ScrollDropdownTemplate", {
...
function lib:CreateSoundDropdown(cat, data)
	...
	local setting = registerSetting(
		...
		local initializer = Settings.CreateElementInitializer("LibGCDCPf093333_SoundDropdownTemplate", {
			setting = setting,
...
function lib:CreateColorOverrides(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibGCDCPf093333_ColorOverridesPanelNoHead", {
...
function lib:CreateMultiDropdown(cat, data)
	...
	local initializer = Settings.CreateElementInitializer("LibGCDCPf093333_MultiDropdownTemplate", {
		label = data.name or data.text or data.key,
		options = data.values,
		optionfunc = data.optionfunc,
		order = data.order,
...
function lib:CreateText(cat, text, extra)
	...
	local init = Settings.CreateElementInitializer(
		"LibGCDCPf093333_SettingsListSectionHintTemplate",
		{ name = name }
	)
	addSearchTags(init, data.searchtags or name, name)
	applyParentInitializer(init, data.parent, data.parentCheck)
...
function lib:SetVariablePrefix(prefix)
	if type(prefix) == "string" and prefix ~= "" then
		if State.prefixSet and prefix ~= State.prefix then
			error(
				("LibGCDCPSettingsMode: prefix already set to '%s'; refusing to change to '%s'. Use data.prefix per control if you need multiple namespaces."):format(
