local MODULE_MAJOR, EXPECTED_MINOR = "LibGCDCPSettingsMode-1.0", 16011001
...
LibGCDCP_ColorOverridesMixin = CreateFromMixins(SettingsListElementMixin)
...
function LibGCDCP_ColorOverridesMixin:OnLoad()
...
function LibGCDCP_ColorOverridesMixin:Init(initializer)
...
function LibGCDCP_ColorOverridesMixin:GetSpacing()
...
function LibGCDCP_ColorOverridesMixin:RefreshRows()
...
function LibGCDCP_ColorOverridesMixin:SetupRow(frame, entry)
	frame.data = entry
	if frame.Text then
...
function LibGCDCP_ColorOverridesMixin:ApplyTextColor(frame)
...
function LibGCDCP_ColorOverridesMixin:ResolveColor(key, useDefault)
	local r, g, b, a
	if useDefault then
...
function LibGCDCP_ColorOverridesMixin:ApplyColor(key, r, g, b, a)
...
function LibGCDCP_ColorOverridesMixin:RefreshRow(frame)
...
function LibGCDCP_ColorOverridesMixin:RefreshAll()
	for _, frame in ipairs(self.colorOverrideFrames or {}) do
		self:RefreshRow(frame)
	end
end

function LibGCDCP_ColorOverridesMixin:ResetToDefaults()
...
function LibGCDCP_ColorOverridesMixin:OpenColorPicker(frame)
	if not (self.setColor or self.setColorMixin) then
		return
	end
...
function LibGCDCP_ColorOverridesMixin:Release()
...
function LibGCDCP_ColorOverridesMixin:EvaluateState()
