...
local waNames = {
    ["Shaman"] = "Luxthos - Shaman",
    ["DeathKnight"] = "Luxthos - Death Knight",
    ["Warrior"] = "Luxthos - Warrior",
    ["Paladin"] = "Luxthos - Paladin",
    ["Mage"] = "Luxthos - Mage",
    ["Rogue"] = "Luxthos - Rogue",
    ["Priest"] = "Luxthos - Priest",
    ["Evoker"] = "Luxthos - Evoker",
    ["Warlock"] = "Luxthos - Warlock",
    ["DemonHunter"] = "Luxthos - Demon Hunter",
    ["Druid"] = "Luxthos - Druid",
    ["Monk"] = "Luxthos - Monk",
    ["Hunter"] = "Luxthos - Hunter",
    ["Consumables"] = "Luxthos - Consumables",
    ["Raid"] = "Undermine (https://wago.io/twwraid2)",
    ["Dungeons"] = "TWW Dungeons (https://wago.io/twwdungeons)",
    ["AW_BattleRes"] = "AW BattleRes",
    ["M_Plus_Keys"] = "Mythic+ !keys The War Within",
    ["Racials_And_Trinkets"] = "Luxthos - Racials & Trinkets",
    ["M_Plus_Timer"] = "M+ Timer ", --yes the space is intentional..
    ["M_Plus_Automark"] = "Mythic+ Auto Marker",
    ["Externals_On_You"] = "Externals on you",
    ["Tank_Frontals"] = "Tank Frontals TWW",

    ["PvP_Queue"] = "PvP Queue",
    ["SkillCapped_BuffsAndDebuffs"] = "Skill Capped Buffs & Debuffs WeakAura Pack",
    ["SkillCapped_ClassBuffs"] = "Skill Capped Class Buffs WeakAura Pack",
    ["Enemy_Battleground_CDs"] = "Skill Capped Enemy Battleground CDs WeakAura Pack",
    ["Gladius_DoT_Tracker"] = "Skill Capped Arena DoTs",
    ["SkillCapped_MissingBuffs"] = "Skill Capped Missing Buffs WeakAura Pack",
    ["Healer_CC"] = "Healer cc (colored borders)",
    ["Drink"] = "Drink",
    ["SkillCapped_EnemyCDs"] = "Skill Capped Enemy CDs WeakAura Pack",

    ["Peacekeeper_Mute"] = "Mute Oathsworn Peacekeeper Duel",
}

local waNamesPvE = {
    ["Consumables"] = "Luxthos - Consumables",
    ["Raid"] = "Undermine Raid", --edit
    ["Dungeons"] = "TWW Dungeons", --edit
    ["AW_BattleRes"] = "AW BattleRes",
    ["M_Plus_Keys"] = "Mythic+ !keys The War Within",
    ["Racials_And_Trinkets"] = "Luxthos - Racials & Trinkets",
    ["M_Plus_Timer"] = "M+ Timer ", --yes the space is intentional..
...
local waNamesPvP = {
    ...
    ["Enemy_Battleground_CDs"] = SC.scIcon.."Enemy BG CDs WeakAura Pack",
    ["Gladius_DoT_Tracker"] = SC.scIcon.."Arena DoTs",
    ["SkillCapped_MissingBuffs"] = SC.scIcon.."Missing Buffs WeakAura Pack",
    ["Healer_CC"] = "Healer in CC",
    ["Drink"] = "Drink",
...
local reloadRequiredWeakAuras = {
    ["Externals_On_You"] = true,
    ["Consumables"] = true,
    ["Racials_And_Trinkets"] = true,
    ["SkillCapped_BuffsAndDebuffs"] = true,
    ["SkillCapped_ClassBuffs"] = true,
    ["Mage"] = true,
    ["Druid"] = true,
    ["Warrior"] = true,
    ["DemonHunter"]  = true,
...
function SC:DeleteWeakAuraAndChildren(rootName)
    if not WeakAurasSaved or not WeakAurasSaved.displays then return end
    local displays = WeakAurasSaved.displays
    if not displays[rootName] then return end

    local toDelete = { [rootName] = true }
    local changed = true
    while changed do
        changed = false
        for name, aura in pairs(displays) do
            if not toDelete[name] and aura and aura.parent and toDelete[aura.parent] then
                toDelete[name] = true
                changed = true
            end
        end
    ...
    for name, aura in pairs(displays) do
        if aura and aura.controlledChildren and not toDelete[name] then
            local filtered = {}
            for _, child in ipairs(aura.controlledChildren) do
                if not toDelete[child] then
                    table.insert(filtered, child)
                end
            end
            aura.controlledChildren = filtered
        end
    end
...
local function SetWeakAuraLoadNeverState(name, state)
    ...
    for childName, childAura in pairs(WeakAurasSaved["displays"]) do
        if childAura["parent"] == name then
            SetWeakAuraLoadNeverState(childName, state)
        end
    end
end
...
function SC:TogglePvEWeakAuras(contentType)
    ...
    if SkillCappedDB.hidePvEWeakAurasInPvP and contentType == "PvP" then
        ...
        for _, className in ipairs(classNames) do
            SetWeakAuraLoadNeverState("Luxthos - " .. className, true)
        end
        for _, waName in ipairs(waNames) do
            SetWeakAuraLoadNeverState(waName, true)
        end
    else
        -- Enable all Luxthos WeakAuras for each class
        for _, className in ipairs(classNames) do
            SetWeakAuraLoadNeverState("Luxthos - " .. className, false)
        end
        for _, waName in ipairs(waNames) do
            SetWeakAuraLoadNeverState(waName, false)
        end
    end

    if SkillCappedDB.hidePvPWeakAurasInPvE and contentType == "PvE" then
        for _, pvpWaName in ipairs(pvpWaNames) do
            SetWeakAuraLoadNeverState(pvpWaName, true)
        end
    else
        for _, pvpWaName in ipairs(pvpWaNames) do
            SetWeakAuraLoadNeverState(pvpWaName, false)
        end
    end
end
                ...
                for weakaura, importString in pairs(categoryWeakAuras) do
                    if (SkillCappedWeakAurasDB[weakaura] and SkillCappedWeakAurasDB[weakaura] ~= importString) then
                        ...
                        if WeakAurasSaved and WeakAurasSaved["displays"] and WeakAurasSaved["displays"][weakauraName] then
                            if not (SkillCappedDB.ignoredWeakAuras and SkillCappedDB.ignoredWeakAuras[weakaura]) then
                                updatesAvailable = true
                                if not SkillCappedDB.twwS3WAUpdate and (weakauraName == "Undermine (https://wago.io/twwraid2)" or weakauraName == "TWW Dungeons (https://wago.io/twwdungeons)") then
                                    SC.weakauraUpdateNeedsReload = true
                                end
                            end
                            SC.WeakAurasWithUpdates[weakaura] = true
                        else
...
function SC:CheckAndPromptMissingClassWeakAuras()
    ...
    if classWeakAuras and not SkillCappedWeakAurasDB[playerClassX] then
        if SkillCappedDB.mainContent == "PvP" and SkillCappedDB.hidePvEWeakAurasInPvP then return end
