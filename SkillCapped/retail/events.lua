...
SC:RegisterEvent("PLAYER_LOGIN", function()
    ...
    if SkillCappedBackupDB.FrameColorDB and not SkillCappedBackupDB.FrameColor4DB then
    ...
    if db.completedSetup and not db.midnightInitialized then
        ...
        StaticPopup_Show("SC_MIDNIGHT_UPDATE")
    end
    ...
    if SkillCappedDB.enabledAddons["ArcUI"] and SkillCappedDB.WeakAura then
        SC.ArcUIEnabled = true
    end

    if SkillCappedDB.mainContent == "PvP" and UnitLevel("player") >= 10 then
    ...
    C_Timer.After(4, function()
        ...
        if SkillCappedDB.editModeIsFull then
            StaticPopup_Show("SC_EDITMODE_FULL")
            SkillCappedDB.editModeIsFull = nil
            SkillCappedDB.editModeFullButHasProfile = nil
        elseif SkillCappedDB.editModeFullButHasProfile then
            local existingProfile = SkillCappedDB.editModeFullButHasProfile
            StaticPopup_Show("SC_EDITMODE_FULL_HAS_PROFILE", existingProfile)
            SkillCappedDB.editModeFullButHasProfile = nil
            SkillCappedDB.editModeIsFull = nil
        end

        if SkillCappedDB.cdmIsFull then
            StaticPopup_Show("SC_CDM_FULL")
            SkillCappedDB.cdmIsFull = nil
            SkillCappedDB.cdmFullButHasProfile = nil
        elseif SkillCappedDB.cdmFullButHasProfile then
            local existingProfile = SkillCappedDB.cdmFullButHasProfile
            StaticPopup_Show("SC_CDM_FULL_HAS_PROFILE", existingProfile)
            SkillCappedDB.cdmFullButHasProfile = nil
            SkillCappedDB.cdmIsFull = nil
        end
    end)
        ...
        if SC:IsRetail() then
            C_Timer.After(3, function()
                ...
                local function SaveEditModeProfile()
                    ...
                    elseif contentType == "PvP" and activeLayout == "SkillCapped" and (SkillCappedBackupDB and SkillCappedBackupDB["EditModePvP"]) then
                        SkillCappedDB.enabledAddons["EditModePvP"] = true
                    elseif contentType == "PvE" and activeLayout == "SkillCapped PvE" and (SkillCappedBackupDB and SkillCappedBackupDB["EditModePvE"]) then
                        SkillCappedDB.enabledAddons["EditModePvE"] = true
                    end
...
function SC:CheckAndPromptProfileSwap()
    ...
    if (currentContent ~= SkillCappedDB.mainContent) then
