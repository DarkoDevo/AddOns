...
function SC:MakeGUI(showUI)
    ...
    function SC:CreateInstallStep(contentType, parentFrame)
        ...
        local yOffset = contentType == "LookAndFeel" and -98 or -100
        local yOffset2 = -53
        ...
        for addonName, cb in pairs(contentCheckboxes) do
            ...
            -- Not loaded addons
            if not SC:IsAddonLoaded(addonName) and addonIsNot("Personal Resource Display", addonName) and addonIsNot("SimpleItemLevel", addonName) and addonIsNot("Bagnon", addonName) and addonIsNot("Combo Points on Nameplate", addonName) then
                cb.notLoaded = true
                if contentType ~= "LookAndFeel" then
                    cb:SetChecked(false)
                end
    ...
    function SC:FinishInstall()
        ...
        if lookAndFeelCheckboxes["Fade Status/XP Bar"] then
            SC:FadeStatusBarTBC(lookAndFeelCheckboxes["Fade Status/XP Bar"]:GetChecked())
        end
        ...
        if lookAndFeelCheckboxes["Class Colored Healthbars"] then
            if not lookAndFeelCheckboxes["Class Colored Healthbars"]:GetChecked() then
                ...
                if BetterBlizzFramesDB then BetterBlizzFramesDB["classColorFrames"] = true end
            end
        end
