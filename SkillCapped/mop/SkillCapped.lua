...
-- local function InitAddon(entry, treeNode)
--     local addonIndex = treeNode:GetData().addonIndex
--     local title = entry.Title
--     local status = entry.Status
--     local currentChar = character--GetAddonCharacter()
...
--     local loadable, reason = C_AddOns.IsAddOnLoadable(addonIndex, currentChar)
...
function SC:SetupSettingsAndGUI()
    ...
    if db.reOpenToAddonConfig or db.openInstallerOnLaunch then
    ...
    C_Timer.After(4, function()
        SC:RunAfterCombat(function()
            ...
            local newWeakAurasAvailable = SC:IsAddonLoaded("WeakAuras") and SC:NewWeakAuraUpdates()

            if (newTalentsAvailable and newWeakAurasAvailable) or newWeakAurasAvailable then
                SC.newTalentsAndWeakAurasAvailable = newTalentsAvailable

                if SC.newMopLaunch then
                    db.updateWANextLaunch = true
                else
                    if db.WeakAuraDel then
                        --StaticPopup_Show("SC_NEW_UPDATE")
                        db.WeakAuraDel = nil
                    else
                        if not db.dontShowWeakAuraUpdateMessage and not SC.setupActive then
                            StaticPopup_Show("SC_WEAKAURA_UPDATE")
                        end
                        if db.updateWANextLaunch then
                            db.updateWANextLaunch = nil
                        end
                    end
                    db.couldNotUpdateWeakAuras = nil
                end
            elseif newTalentsAvailable then
                if not db.dontShowTalentUpdateMessage and not SC.setupActive then
                ...
                if not SkillCappedDB.dontShowWeakAuraClassUpdateMessage then
                    SC:CheckAndPromptMissingClassWeakAuras()
                end
            end
        end)
    end)
...
function SC:ChangeWeakAura(weakaura)
    ...
    if importString then
        ...
        if (playerBattleNetIDLower..addonConfig == importStringLower or playerBattleNetID..addonConfig == importString) then
...
function SC:CheckAddonUpdates()
    ...
    if importString then
        ...
        local importStringLower = string.lower(importString)
        if concatenatedString ~= importStringLower and concatenatedString2 ~= importStringLower then
...
function SC:CreateMinimapIcon()
    ...
    local skillCappedDataObject = LDB:NewDataObject("SkillCappedMinimapIcon", {
        ...
        OnTooltipShow = function(tooltip)
            ...
            local secondaryContentText = SkillCappedDB.secondaryContent or "none"
            ...
            -- else
            --     tooltip:AddLine("Right-click will swap to secondary UI when installed.")
            -- end
        end,
    })
