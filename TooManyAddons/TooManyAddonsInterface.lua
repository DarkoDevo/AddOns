...
TMA_CHECKBUTTON_RIGHT_OFFSET = -5
...
----------------- load addon button
 	...
 	--------------------------- game menu button --------------------------------------
		TMAgamemenubutton = CreateFrame("Button","GameMenuButtonTMAAddOns",GameMenuFrame,"GameMenuButtonTemplate")
			...
			TMAgamemenubutton:SetPoint("TOP",GameMenuFrame,"BOTTOM",0,-1)
			TMAgamemenubutton:SetPoint("CENTER",GameMenuFrame, "CENTER",0,-1)
...
function TMAcreatepopupoptions()  --not in use yet
   ...
   --TMApopupoptions:SetBackdropColor(0,0,0,2,0) -- Removed in 9.0.1, broken in 10.0.0
...
function TMAcreateoptionframe()
	...
	Button = CreateFrame("CheckButton", "TMAoption1", Frame, "UICheckButtonTemplate");
	...
	--option 2 - No memory usage
	Button = CreateFrame("CheckButton", "TMAoption2", Frame, "UICheckButtonTemplate");
	Button:SetPoint("TOPLEFT",TMAoption1,"BottomLeft",0, -25);
	getglobal(Button:GetName().."Text"):SetText("Disable memory usage statistics");
	Button.tooltipText = "Disable addon memory information.  Eliminates certain (inaccurate) warnings about performance (stuttering)."
	Button:SetScript("OnClick",function(self)
		if self:GetChecked()then
			-- PlaySound("igMainMenuOptionCheckBoxOff");
			TMAhidememory = true
	...
	Button = CreateFrame("CheckButton", "TMAoption3", Frame, "UICheckButtonTemplate");
	...
	Button = CreateFrame("CheckButton", "TMAoption4", Frame, "UICheckButtonTemplate");
  ...
  Text:SetText("Version "..tostring(C_AddOns.GetAddOnMetadata("TooManyAddons","Version")));
...
function TMAcreateaddonbutton(i)
    local currentcheckbutton,currentalwaysbutton,currentcollapsebutton,currentbutton
TMAprint("TMAcreateaddonbutton(" .. i .. ")");

	-- "Always" button - reflects state of addon in "Always Load These Addons"
	currentalwaysbutton = TMACreateAlwaysButton(TMA_ADDON_LIST_NAME,i)
    currentalwaysbutton:SetScript("OnClick",function(self)
		TMAaddonlistalwaysbutton_onclick(self)
	end)

	currentalwaysbutton:SetScript("OnEnter",function(self)
        TMAShowAddonAlwaysTooltip(self)
    end)
    currentalwaysbutton:SetScript("OnLeave",function()
        TMAHideAddonAlwaysTooltip()
    end)

	-- "Current check box" - Reflects state of addon in current profile(s)
...
--edited		self:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestLogTitleHighlight")  --in case moving the button disabled it temorarily
        TMAhidetooltip()
    end)
...
function TMAcreatechecklist(name)
	...
	-- make the global frame
		...
		-- ourframe:SetBackdropColor(0,0,0,1,1) -- Removed in 9.0.1, broken in 10.0.0
		ourframe:SetClampedToScreen(true)
		ourframe:SetResizable(true)
--		ourframe:SetMaxResize(myFrameWidth+50,600)    -- Removed 10.0.0
--		ourframe:SetMinResize(TMA_FRAME_WIDTH,50)    -- Removed 10.0.0
...
function TMACreateAlwaysButton(name,i)
TMAprint("TMACreateAlwaysButton(" .. name .. "," .. i .. ")")
	local ourframe        = getglobal(name.."frame")
	local ouralwaysbutton = CreateFrame("CheckButton",name.."alwaysbutton"..i,ourframe, "UICheckButtonTemplate")

	ouralwaysbutton:SetCheckedTexture("Interface\\Buttons\\UI-CheckBox-Check-Disabled");

	ouralwaysbutton:SetHeight(TMA_HEIGHT_OF_BUTTON)
	ouralwaysbutton:SetWidth(ouralwaysbutton:GetHeight())
	ouralwaysbutton:SetNormalFontObject(GameFontNormal)
	ouralwaysbutton.number = i

		-- if its the top of a row
	if (i == 1) then
			--its the first button
			ouralwaysbutton:SetPoint("TOPLEFT",13,-13)
	else
		local previousrow= getglobal(name.."alwaysbutton"..(i-1))
		ouralwaysbutton:SetPoint("TOP",previousrow,"BOTTOM",0,TMA_GAP_BETWEEN_BUTTONS)
	end

	return ouralwaysbutton;
end

function TMACreateCheckButton(name,i)
	local ourframe = getglobal(name.."frame")
	local ourcheckbutton = CreateFrame("CheckButton",name.."checkbutton"..i,ourframe, "UICheckButtonTemplate")
	...
	if (name == TMA_PROFILE_LIST_NAME) then
		if (i == 1) then
			ourcheckbutton:SetPoint("TOPLEFT",13,-13)
		else
			local previousrow= getglobal(name.."checkbutton"..(i-1))
			ourcheckbutton:SetPoint("TOP",previousrow,"BOTTOM",0,TMA_GAP_BETWEEN_BUTTONS)
		end
	else
		local ouralwaysbutton = getglobal(name.."alwaysbutton"..i)
		ourcheckbutton:SetPoint("TOPLEFT",ouralwaysbutton,"TOPRIGHT", TMA_CHECKBUTTON_RIGHT_OFFSET, 0)
	end
