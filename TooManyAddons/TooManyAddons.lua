	--[[ https://warcraft.wiki.gg/wiki/World_of_Warcraft_API#Patch_10.2.0
	DEFAULT_CHAT_FRAME:AddMessage("")
	C_AddOns.DisableAddOn(name [, character]) - Disables an addon on the next session.
	C_AddOns.DisableAllAddOns([character]) - Disables all addons on the addon list.
	C_AddOns.DoesAddOnExist(name) : exists
	C_AddOns.EnableAddOn(name [, character]) - Enables an addon on the next session.
	C_AddOns.EnableAllAddOns([character]) - Enables all addons on the addon list.
	C_AddOns.GetAddOnDependencies(name) : unpackedPrimitiveType - Returns a list of TOC dependencies.
	C_AddOns.GetAddOnEnableState(name [, character]) : state
	C_AddOns.GetAddOnInfo(name) : name, title, notes, loadable, reason, security, updateAvailable - Get information about an AddOn.
	C_AddOns.GetAddOnOptionalDependencies(name) : unpackedPrimitiveType - Returns a list of optional TOC dependencies.
	C_AddOns.GetNumAddOns() : numAddOns - Get the number of user supplied AddOns.
	C_AddOns.IsAddOnLoadOnDemand(name) : loadOnDemand - Returns true if the specified addon is load-on-demand.
	C_AddOns.IsAddOnLoadable(name [, character, demandLoaded]) : loadable, reason
	C_AddOns.IsAddOnLoaded(name) : loadedOrLoading, loaded - Returns true if the specified addon is loaded.
	C_AddOns.IsAddonVersionCheckEnabled() : isEnabled
	C_AddOns.LoadAddOn(name) : loaded, value - Loads the specified LoadOnDemand addon.
	C_AddOns.ResetAddOns()
	C_AddOns.ResetDisabledAddOns()
	C_AddOns.SaveAddOns()
	C_AddOns.SetAddonVersionCheck(enabled)

	name, title, notes, enabled, loadable, reason, security = C_AddOns.GetAddOnInfo(index or "name")
	]]
...
function TMA_onload()
	...
	if(C_AddOns.GetNumAddOns() > 100) then
		DEFAULT_CHAT_FRAME:AddMessage(C_AddOns.GetNumAddOns().." addons found.  You have TooManyAddons! ('/TMA help' for more info)")
	else
		DEFAULT_CHAT_FRAME:AddMessage("Only "..C_AddOns.GetNumAddOns().." addons found.  Get some more! ('/TMA help' for more info)")
	end
 	--end loading
   ...
   if not TMAsettings.servers then
	...
	if (TMAsettings.dates["toomanyaddons"]) then  -- Needed due to module name case change.  Give it a year to propagate.  Take it out in 2022
		...
		for i = 1,C_AddOns.GetNumAddOns() do
       	name,title,_, _, _, _, _ = C_AddOns.GetAddOnInfo(i)
...
--TMAprofiles = TMAsettings.servers[serverName][playerName]  --this is our local copy
	...
	if (not theonetable[1]) then
		...
		for i=1, C_AddOns.GetNumAddOns() do
			name,title,_, enabled, _, _, _ = C_AddOns.GetAddOnInfo(i)
...
function TMAinitializeoptions()
	...
	if (TMAsettings.hidememory) then
		TMAhidememory = TMAsettings.hidememory
	else
		TMAhidememory = false
	end
	TMAoption2:SetChecked(TMAhidememory)
...
function TMAcreatesortingmethods()
    ...
    ourstruct.func = function(a,b)
       ...
       local Aauthor = C_AddOns.GetAddOnMetadata(a.name, "Author") or ""
       local Bauthor = C_AddOns.GetAddOnMetadata(b.name, "Author") or ""
    ...
    ourstruct.func = function(a,b)
		...
		local _, _, _, aenabled, _, _, _ = C_AddOns.GetAddOnInfo(a.name)
		local _, _, _, benabled, _, _, _ = C_AddOns.GetAddOnInfo(b.name)
		if(TMAsortdesc) then
...
function TMA(input)
	if not (TMAhidetooltips) then
		if not (TMAhidememory) then
			TMAprint("|c00ffbb00event refresh addon memory usage")
	...
	if(input) then
		...
		elseif (input == "hidememory") then
			TMAhidememory = not TMAhidememory
			DEFAULT_CHAT_FRAME:AddMessage("TMA Memory Hidden?:".."|c00ff00ff"..tostring(TMAhidememory).."|r")
...
function TMAupdateaddonframe()
	...
	local currentcollapsebutton,currentalwaysbutton,currentcheckbutton,currentbutton, value, ratio
	ratio = {-.2,1,-.1,1.1}
	...
	for i=1,rtcf do
		...
		currentalwaysbutton = getglobal(TMA_ADDON_LIST_NAME.."alwaysbutton"..i)
		currentcheckbutton = getglobal(TMA_ADDON_LIST_NAME.."checkbutton"..i)
...
--			currentcheckbutton:GetDisabledTexture():SetTexCoord(0,1,0,1)
		...
		if (isthealwaysselected) then
			currentalwaysbutton:Hide();
		else
			currentalwaysbutton:Show();
			currentalwaysbutton:SetChecked(TMAischeckedinalways(addonname));
		end


		currentcheckbutton:SetChecked(TMAisshowcheck(addonname))
...
--create addon display list
    ...
    for i=1,C_AddOns.GetNumAddOns() do
        name,title,notes, _, _, _, _ = C_AddOns.GetAddOnInfo(i)
		...
		if (TMAisheader(tempaddondisplaylist[j].name)) then
			...
			if not (TMAgroups[prefix].collapsed) then
				for i = 2, #TMAgroups[prefix].kids do  --this stuff was set up when we created TMAgroups
				   ...
				   local name,title,notes, _, _, _, _ = C_AddOns.GetAddOnInfo(addonnumber)
				   ourstruct = {}
				   ourstruct.name = name
				   ourstruct.title = title
				   ourstruct.notes = notes
...
function TMAisdisabled(addonname)
    ...
    local enabled
    _,_,_,enabled = C_AddOns.GetAddOnInfo(addonname)
    if(enabled) then
        return false
    else
        return true
...
function TMAsavesettings()
   ...
   TMAsettings.hidememory = TMAhidememory
...
function TMAloadprofile(profile)
	...
	if (thealwaysprofilenum) then  --they might have deleted it - but it will be back mwahahahahah
	   for i=1,C_AddOns.GetNumAddOns() do
		  name,_,_, _, _, _, _ = C_AddOns.GetAddOnInfo(i)
		...
		for i=1,#theonetable do
			if(theonetable[i].isselected) then
				for j=1,C_AddOns.GetNumAddOns() do
					name,_,_, _, _, _, _ = C_AddOns.GetAddOnInfo(j)
					if(theonetable[i][name]) then
						C_AddOns.EnableAddOn(j, UnitName("player"))
					end
				end
			end
		end
	...
	elseif(TMAdebug) then
		...
		--TMAprint(theonetable)
		--C_AddOns.EnableAddOn("TooManyAddons", UnitName("player"))
	else
		ReloadUI()
...
function TMAHideAddonAlwaysTooltip()
    GameTooltip:Hide()
end

function TMAShowAddonAlwaysTooltip(frame)
TMAprint("Entering always tooltip..")
	if (TMAhidetooltips) then
		return
	end
TMAprint("Printing always tooltip..")

	GameTooltip:SetOwner(frame)
	GameTooltip:AddLine("The state of this box is controlled by the '" .. TMAALWAYSPROFILE .. "' profile.  If checked, it always overrides the checkbox immediately to the right.",1,1,1,1)
	GameTooltip:Show()
...
--Maintainer's note many years later.. "Hmmmm"
function TMAShowAddonTooltip(frame)
	if not (TMAhidetooltips) then
		local index = frame.addonnumber
		if not index then return end
		local name, title, notes, enabled, loadable, reason, security = C_AddOns.GetAddOnInfo(index)
		...
		if TMAhidememory then
			GameTooltip:AddLine(string.format("%s: %s", CLR:Label("Memory Usage"), CLR:Red("Turned off in Options")))
		else
...
function TMAaddonlistalwaysbutton_onclick(self)
	TMAupdate();
end
...
function TMAsetdependencies(addonname,profilenum)
	...
	local deps = {}
	local profile
	deps = {C_AddOns.GetAddOnDependencies(addonname)}
...
function TMAonenterfunction(self)
	...
	if(TMAmovingbutton) then  --mouse is being held down
		...
		-- TMAmovetoindicator:SetBackdropColor(1,1,1,1,1) -- Removed in 9.0.1, broken in 10.0.0
