...
function Appraiser.RecordManualSellPrice(auction)
	if not Appraiser.ActiveSellingItem then return end

	local cInput = AuctionHouseFrame.CommoditiesSellFrame.PriceInput.MoneyInputFrame
	local iInput = AuctionHouseFrame.ItemSellFrame.PriceInput.MoneyInputFrame

	if auction then
		table.wipe(Appraiser.ManualSellPrice)
		Appraiser.ManualSellPrice[1] = Appraiser.ActiveSellingItem.link
		Appraiser.ManualSellPrice[2] = auction.unitPrice or auction.buyoutAmount
	elseif cInput.GoldBox:HasFocus() or cInput.SilverBox:HasFocus() or cInput.CopperBox:HasFocus() then
		table.wipe(Appraiser.ManualSellPrice)
		local buyoutPrice = AuctionHouseFrame.CommoditiesSellFrame.PriceInput:GetAmount();
		Appraiser.ManualSellPrice[1] = Appraiser.ActiveSellingItem.link
		Appraiser.ManualSellPrice[2] = buyoutPrice
	elseif iInput.GoldBox:HasFocus() or iInput.SilverBox:HasFocus() or iInput.CopperBox:HasFocus() then
		table.wipe(Appraiser.ManualSellPrice)
		local buyoutPrice = AuctionHouseFrame.ItemSellFrame.PriceInput:GetAmount();
		Appraiser.ManualSellPrice[1] = Appraiser.ActiveSellingItem.link
		Appraiser.ManualSellPrice[2] = buyoutPrice
	end
end
...
function Appraiser:AddItemToInventory(bag,slot)
	...
	local active = false
	if Appraiser.ActiveSellingItem and Appraiser.ActiveSellingItem.itemlink == stripped_link then active = true end
	...
	for i,v in pairs(self.InventoryItems) do
		...
		if v.link==stripped_link then
			local invcached =  self.InventoryItems[i]
			if invcached.maxStack > 1 then
				exists = true
				invcached.count = invcached.count + itemInfo.stackCount
				invcached.price = invcached.price + price
				break
			end
		end
	end
	if not exists then
		...
		invcached.damaged = current~=maximum
...
function Appraiser:UpdateItemInInventory(item)
	...
	if item.damaged then
		item.statusText="Item damanged, cannot be sold"
		item.statusIcon=ZGV.IconSets.AuctionToolsPriceIcons.NOPE.texcoord
		item.statusColor={1,0,0,1}
		item.statusId=99
	end
end
...
function Appraiser:ActivateSellItem(item,automatic)
	...
	if item.damaged then return end
	...
	if not Appraiser.SellingInProgress then
		if AuctionHouseFrame.CommoditiesSellFrame:IsVisible() then
			...
			if item.link == Appraiser.ManualSellPrice[1] then
				price = Appraiser.ManualSellPrice[2]
			else
				table.wipe(Appraiser.ManualSellPrice)
			end

			ZGV:ScheduleTimer(function() 
				AuctionHouseFrame.CommoditiesSellFrame.QuantityInput:SetQuantity(item.count)
				AuctionHouseFrame.CommoditiesSellFrame.PriceInput:SetAmount(price)
			end,0)
		elseif AuctionHouseFrame.ItemSellFrame.ItemDisplay:IsVisible() then
			...
			MoneyInputFrame:SetAmount(price)
		end
	end
...
function Appraiser:SelectNextSellingItem()
	...
	if not refresh and not AuctionHouseFrame.CommoditiesSellFrame.ItemDisplay:GetItem() then
		local item = Appraiser.InventoryItems[index]

		local itemid=C_Container.GetContainerItemID(item.bag, item.slot)
