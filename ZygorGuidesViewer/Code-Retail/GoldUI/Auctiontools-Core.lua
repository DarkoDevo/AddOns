...
local function Appraiser_SetTooltipData(tooltip)
	...
	if not Appraiser.TooltipPatched then
		...
		for i=1,tooltip:NumLines() do
			...
			if line then
				...
				isboundable = isboundable or line==ITEM_ACCOUNTBOUND_UNTIL_EQUIP
				isboundable = isboundable or line==ITEM_ACCOUNTBOUND
				isboundable = isboundable or line==ITEM_BIND_TO_ACCOUNT_UNTIL_EQUIP
				isboundable = isboundable or line==ITEM_BIND_TO_BNETACCOUNT
		...
		if isboundable then
			tooltip:AddLine("  |cffeeeeeeItem is not tradeable|r ")
			tooltip:Show()
			return
		end
...
function Appraiser.EventHandler(frame, event, ...)
	...
	if event=="ADDON_LOADED" and ...=="Blizzard_AuctionHouseUI" then
		...
		hooksecurefunc(AuctionHouseFrame.CommoditiesSellFrame,"UpdatePostState",function() Appraiser.RecordManualSellPrice() end)
		hooksecurefunc(AuctionHouseFrame.CommoditiesSellFrame,"OnAuctionSelected",function(_,auction) Appraiser.RecordManualSellPrice(auction) end)

		hooksecurefunc(AuctionHouseFrame.ItemSellFrame,"UpdatePostState",function() Appraiser.RecordManualSellPrice() end)
		hooksecurefunc(AuctionHouseFrame.ItemSellFrame,"OnSearchResultSelected",function(_,auction) Appraiser.RecordManualSellPrice(auction) end)
	...
	elseif event=="ITEM_COUNT_CHANGED" then
		...
		local activeitem = ZGV.Gold.Appraiser.ActiveSellingItem
		if activeitem and itemid == activeitem.itemid and activeitem.count>0 then
...
function Appraiser.GetTrueItemCount(item)
	-- can't use C_Item.GetItemCount since it doesn't differentiate by link properly
	local truecount = 0
	for bag=0, 5 do  -- NUM_BAG_SLOTS is 4, but reagent bag is 5
		for slot=1, C_Container.GetContainerNumSlots(bag) do
			local itemInfo = C_Container.GetContainerItemInfo(bag, slot)
			if itemInfo and item.link == ZGV.ItemLink.StripBlizzExtras(itemInfo.hyperlink) then
...
function Appraiser:CreateTrackerFrame()
	...
	Appraiser.Events:RegisterEvent("ITEM_COUNT_CHANGED")
	...
	TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Item, Appraiser_SetTooltipData)
