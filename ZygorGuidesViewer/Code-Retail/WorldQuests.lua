...
function WorldQuests:SuggestWorldQuestGuide(object,questID,force,mapID)
	local questID = object and (object.worldQuest or object.isCombatAllyQuest or object.isDaily) and object.questID or questID or object.vignetteID
	...
	if C_QuestLog.GetQuestWatchType(questID) or (object and (object.isCombatAllyQuest or object.hiddenworldquest or object.vignetteID)) or force then
		...
		if not labelstep then
			if object and object.questID and object.worldQuest then
...
local function get_quest_details(data,qid)
	...
	for factionID,factionData in pairs(faction_data) do
		if (C_QuestLog.DoesQuestAwardReputationWithFaction(qid, factionID) or C_QuestLog.IsQuestCriteriaForBounty(qid, factionData.quest) or (data.mapID==factionData.zone) or (data.faction==factionID)) and (not factionData.faction or factionData.faction==player_faction) then
			if faction_data[factionID].filepath then
				data.reputations =  data.reputations..("|TInterface\\Icons\\"..faction_data[factionID].filepath..":20:20:0:4|t " or faction_data[factionID].name or "")
