...
if DBM then
	...
	DBM:RegisterCallback("DBM_TimerBegin", function(_, id, msg, timer, icon, simpType, spellid, colorId)
		if not Timers[id] then
			Timers[id] 				= {
				text 				= strlowerCache[msg],
				start 				= TMW.time,
				duration 			= timer,
			}
		else
			Timers[id].text 		= strlowerCache[msg]
			Timers[id].start 		= TMW.time
			Timers[id].duration 	= timer
		end
