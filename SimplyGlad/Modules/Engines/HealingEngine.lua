...
local function untaintNumber(value, fallback)
    local requestedFallback = fallback or 0

    if type(Compat.UntaintNumber) == "function" then
        local ok, result = pcall(Compat.UntaintNumber, Compat, value, requestedFallback)
        if ok and type(result) == "number" then
            return result
        end

        ok, result = pcall(Compat.UntaintNumber, value, requestedFallback)
        if ok and type(result) == "number" then
            return result
        end
    end
    ...
    if type(native) == "function" then
        local ok, result = pcall(native, value, requestedFallback)
        if ok and type(result) == "number" then
            return result
        end
    end
...
A_Unit = function(unitID)
        if A and A.Unit then
                local ok, unit = pcall(function() return A.Unit(unitID) end)
                if ok and unit then
                        return unit
                end
        end
...
A_IsUnitFriendly = function(unitID)
        local ok, result = pcall(A.IsUnitFriendly, unitID)
        return ok and result or false
end

A_IsUnitEnemy = function(unitID)
        local ok, result = pcall(A.IsUnitEnemy, unitID)
        return ok and result or false
end

A_PauseChecks = function(...)
        local ok, result = pcall(A.PauseChecks, ...)
        return ok and result or false
end

A_GetLatency = function(...)
        local ok, result = pcall(A.GetLatency, ...)
        return ok and result or 0
end

A_GetCurrentGCD = function(...)
        local ok, result = pcall(A.GetCurrentGCD, ...)
        return ok and result or 0
end

A_GetGCD = function(...)
        local ok, result = pcall(A.GetGCD, ...)
        return ok and result or 0
end

A_GetSpellDescription = function(...)
        local ok, result = pcall(A.GetSpellDescription, ...)
	...
	if not A.IsBasicProfile and A.IsGGLprofile and A.CurrentProfile ~= "[GGL] Template" then 
		...
		if A_Unit(player):HasSpec(CONST.MONK_MISTWEAVER) then 		
			...
			if MK and MK.IsDetoxAble then -- 'MK.IsDetoxAble' skips profiles with version which is lower than 15
				...
				-- Dispel 
				if self.needDispel and not QueueOrder.useDispel[Role] and (not A.IsInPvP or not isSelf) and MK.IsDetoxAble(unitID, untaintNumber(A_Unit(unitID):TimeToDie(), 0), nil, true) then 
					QueueOrder.useDispel[Role] = true 
				...
				-- Utils
				if self.useUtils and not QueueOrder.useUtils[Role] and MK.IsTigersLustAble(unitID, untaintNumber(A_Unit(unitID):TimeToDie(), 0)) then 
...
do 
	local unitMethods					= { __index = {
		CanSelect						= function(self, unitID)
			...
			-- Midnight 12.0: Simplified CanSelect to bypass taint issues
			-- Only check if unit is in range and is not an enemy
			local ok, result = pcall(function()
				return A_Unit(unitID):InRange() and not A_Unit(unitID):IsEnemy()
			end)
			
			-- If pcall failed, still allow the unit through (fallback for NPC/Vehicle units)
			if not ok then
				return true
			end
		...
		Setup							= function(self, unitID, unitGUID, isPlayer)
			...
			local rawAHP = tonumber(untaintNumber(A_Unit(unitID):Health(), 0)) or 0
			local rawMHP = tonumber(untaintNumber(A_Unit(unitID):HealthMax(), 0)) or 0
			local rawPercent = rawMHP > 0 and (100 * rawAHP / rawMHP) or 0
			self.realHP				= rawPercent
			self.realAHP			= rawAHP
			self.MHP				= rawMHP
			...
			if (not self.isPet or db.SelectPets) and (not A_Unit(unitID):IsDead() or self:CanRessurect()) and self:CanSelect() then 					
				local incomingHeals		= PredictOptions[1] and untaintNumber(A_Unit(unitID):GetIncomingHeals()) or 0
				local incomingDMG		= PredictOptions[2] and untaintNumber(A_Unit(unitID):GetRealTimeDMG())  or 0				
				local absorbPossitive 	= PredictOptions[5] and untaintNumber(A_Unit(unitID):GetAbsorb())		  or 0
				local absorbNegative	= PredictOptions[6] and untaintNumber(A_Unit(unitID):GetTotalHealAbsorbs()) or 0
				...
				-- Multiplier - Threat 
				if not A.IsInPvP and untaintNumber(A_Unit(unitID):ThreatSituation()) >= 3 then 
...
local function OnUpdate()   
	...
	-- Midnight fix: TMW_ACTION_GROUP_UPDATE may not have fired yet.
	-- If inGroup is nil but TeamCacheFriendly has group data, refresh it.
	if not inGroup and TeamCacheFriendly.Type then
...
local function SetColorTarget()
	...
	if 	healingTarget == none or healingTargetGUID == none or
		-- /target mode
		(not isFocusHealing and UnitIsUnit(healingTarget, target)) or
		-- /focus mode
		(isFocusHealing and UnitIsUnit(healingTarget, focus))
	then			
	...
	if GetToggle(8, "HealingEngineAPI") then
		if not Data.IsRunning then 
			...
			frame:SetScript("OnUpdate", function(self, elapsed)
				...
				UPD_INTV 		= UPD_INTV > 0.15 and UPD_INTV or 0.15
...
function HealingEngine.SetTarget(unitID, delay)
	...
	healingTarget			= healingTargetGUID and TeamCacheFriendlyGUIDs[healingTargetGUID]
	if healingTarget and not wasSet then 
		healingTargetDelay 	= TMW.time + (delay or 0.5)
		frame:SetColor(healingTarget)		 
	end 
...
function HealingEngine.GetIncomingHPS()
    ...
    for _, thisUnit in ipairs(SortedUnitIDs) do
		total = total + untaintNumber(A_Unit(thisUnit.Unit):GetHEAL(), 0)
	end
...
function HealingEngine.GetTimeToFullDie()
	...
	for _, thisUnit in ipairs(SortedUnitIDs) do
		total = total + untaintNumber(A_Unit(thisUnit.Unit):TimeToDie(), 0)
	end
...
function HealingEngine.GetTimeToDieUnits(timer)
	...
	for _, thisUnit in ipairs(SortedUnitIDs) do
		if untaintNumber(A_Unit(thisUnit.Unit):TimeToDie(), 0) <= timer then
			total = total + 1
		end
	end 
...
function HealingEngine.GetTimeToDieMagicUnits(timer)
	...
	for _, thisUnit in ipairs(SortedUnitIDs) do
		if untaintNumber(A_Unit(thisUnit.Unit):TimeToDieMagic(), 0) <= timer then
			total = total + 1
		end
	end 
	...
	for bossGUID, bossHolders in pairs(BossIDs) do
		if type(bossHolders) == "table" then
			for bossUnitID in pairs(bossHolders) do
				bossHealth = untaintNumber(A_Unit(bossUnitID):Health())
				if bossHealth > 0 then
					healthCurrent = healthCurrent + bossHealth
					healthMax = healthMax + untaintNumber(A_Unit(bossUnitID):HealthMax())
					c = c + 1
					break
				end
			end 
...
function HealingEngine.GetBossTimeToDie()
	...
	for bossGUID, bossHolders in pairs(BossIDs) do 
		if type(bossHolders) == "table" then 
			for bossUnitID in pairs(bossHolders) do 
				ttd = untaintNumber(A_Unit(bossUnitID):TimeToDie(), 0)
				if ttd > 0 then 
					curTTD = curTTD + ttd
					c = c + 1
					break 
...
function HealingEngine.IsManaSave(unitID)
	...
	if db.ManaManagementManaBoss >= 0 then 
		...
		local manaP  = untaintNumber(A_Unit(player):PowerPercent(), 0)
		if bossHP > 0 and manaP <= bossHP and manaP <= db.ManaManagementManaBoss and A_Unit(player):HasBuffs(Aura.Innervate) == 0 then 
			-- Check stop conditions 
			return not unitID or (untaintNumber(A_Unit(unitID):HealthPercent(), 100) >= db.ManaManagementStopAtHP and untaintNumber(A_Unit(unitID):TimeToDie(), 0) >= db.ManaManagementStopAtTTD)
		end 
	end 
end 
