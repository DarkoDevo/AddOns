...
local function GetGUID(unitID)
	local guid = (unitID and (TeamCacheFriendlyUNITs[unitID] or TeamCacheEnemyUNITs[unitID])) or UnitGUID(unitID)
	if guid and _G.issecretvalue and _G.issecretvalue(guid) then return nil end
	return guid
end
...
local COMBAT_LOG_EVENT_UNFILTERED 				= function(...)
	...
	if UnitTrackerIsEventIsDied[EVENT] then
		...
		if spellName then
			local ok, result = pcall(function() return UnitTrackerIsBlockedForTracker[spellName] end)
			if ok then spellBlocked = result end
		end
		if firstFive == "SPELL" and not spellBlocked then
...
local UNIT_SPELLCAST_SUCCEEDED					= function(...)
	...
	if SE then
		if type(SE.UnwrapAuraField) == "function" then
			spellID = SE:UnwrapAuraField(spellID)
		elseif type(SE.NormalizeValue) == "function" then
			local normalized = SE:NormalizeValue(spellID)
			if normalized ~= nil or spellID == nil then
				spellID = normalized
			end
		end
	end
	local blocked = false
	if spellID then
		local ok, result = pcall(function() return UnitTrackerIsBlockedForTracker[spellID] end)
		if ok then blocked = result end
	end
	if unitID and not blocked then
...
A.CombatTracker									= {
	...
	CombatTime									= function(self, unitID)
		...
		local guidSec = _G.issecretvalue and GUID and _G.issecretvalue(GUID)

		if guidSec then
			local uacOk, uac = pcall(UnitAffectingCombat, unit)
			if uacOk then
				if _G.issecretvalue and _G.issecretvalue(uac) then return 0.1, nil end
				if uac then return 0.1, nil end
			end
			return 0, nil
		end
		...
		if CombatTrackerData[GUID] and CombatTrackerData[GUID].combat_time ~= 0 then
			...
			if UnitIsUnit(unit, "player") and InCombatLockdown() then
				inCombat = true
			else
				local uacOk2, uac2 = pcall(UnitAffectingCombat, unit)
				if uacOk2 then
					if _G.issecretvalue and _G.issecretvalue(uac2) then
						inCombat = true
					elseif uac2 then
						inCombat = true
					end
				...
				if not inCombat then
					local ok3, hc = pcall(function() return UnitHasCombat(unit) end)
					if ok3 and hc then inCombat = true end
					if not inCombat then
						local ok4, dummy = pcall(function() return A_Unit(unit):IsDummy() end)
						if ok4 and dummy then inCombat = true end
					end
				end
			end
			if inCombat then
		...
		local uacOk, uac = pcall(UnitAffectingCombat, unit)
		if uacOk then
			if _G.issecretvalue and _G.issecretvalue(uac) then
				if GUID then
					CombatTracker:AddToData(GUID, TMW.time)
				end
				return 0.1, nil
			...
			if uac then
				if GUID then
					CombatTracker:AddToData(GUID, TMW.time)
					if CombatTrackerData[GUID] then
						local elapsed = TMW.time - CombatTrackerData[GUID].combat_time
						return elapsed > 0 and elapsed or 0.1, GUID
					end
				end
				return 0.1, nil
			end
	...
	GetRealTimeDMG								= function(self, unitID)
		...
		if combatTime > 0 and GUID and CombatTrackerData[GUID] and TMW.time - CombatTrackerData[GUID].DMG_lastHit_taken <= A_GetGCD() * 2 + 1 then
	...
	GetRealTimeDPS								= function(self, unitID)
		...
		if combatTime > 0 and GUID and CombatTrackerData[GUID] and TMW.time - CombatTrackerData[GUID].DMG_lastHit_done <= A_GetGCD() * 2 + 1 then
	...
	GetDMG										= function(self, unitID)
		...
		if combatTime > 0 and GUID and CombatTrackerData[GUID] and TMW.time - CombatTrackerData[GUID].DMG_lastHit_taken <= 5 then
	...
	GetSchoolDMG								= function(self, unitID)
		...
		if combatTime > 0 and GUID and CombatTrackerData[GUID] and CombatTrackerData[GUID].School then
			local timestamp = TMW.time
	...
	GetAbsorb									= function(self, unitID, spell)
		-- @return number
		if not spell then
			local ok, val = pcall(UnitGetTotalAbsorbs, unitID)
			if ok and val then
				local isv = _G.issecretvalue
				if isv and isv(val) then return 0 end
				return val
			end
			return 0
		else
	...
	TimeToDieX									= function(self, unitID, X)
		...
		if A.Zone ~= "none" or not A_Unit(UNIT):IsDummy() then
			...
			if (DMG or 0) <= 0 and UnitIsUnit(UNIT, "player") then
				local SE = _G.ActionSecretEngine
				if SE and SE.GetPlayerDamagePerSec then
					DMG, Hits = SE:GetPlayerDamagePerSec()
				end
			end
			DMG = DMG or 0
			Hits = Hits or 0
			...
			local health = nil
			local maxHP = nil
			local hok, hval = pcall(function() return A_Unit(UNIT):Health() end)
			if hok and hval then
				local isv = _G.issecretvalue
				if not isv or not isv(hval) then health = hval end
			end
			if not health then
				local SE = _G.ActionSecretEngine
				if SE and SE.GetHealth then
					local sok, sval = SE:GetHealth(UNIT)
					if sok and sval then health = sval end
				end
			end
			local mok, mval = pcall(function() return A_Unit(UNIT):HealthMax() end)
			if mok and mval then
				local isv = _G.issecretvalue
				if not isv or not isv(mval) then maxHP = mval end
			end

			if health and health <= 0 then return 0 end
			if health and maxHP and DMG >= 1 and Hits > 1 then
				local result = (health - (maxHP * (X / 100))) / DMG
				if result and result >= 0 then ttd = result end
			end
		end
	...
	TimeToDie									= function(self, unitID)
		...
		if A.Zone ~= "none" or not A_Unit(UNIT):IsDummy() then
			...
			if health and health <= 0 then return 0 end
			if health and DMG >= 1 and Hits > 1 then
				local result = health / DMG
				if result and result >= 0 then ttd = result end
			end
		end
		...
		if A.Zone ~= "none" or not A_Unit(UNIT):IsDummy() then
			...
			if (DMG or 0) <= 0 and UnitIsUnit(UNIT, "player") then
				...
				if SE and SE.GetPlayerDamagePerSec then
					...
					if seDmg and seDmg > 0 then DMG = seDmg; Hits = seHits end
				end
			end
			DMG = DMG or 0
			Hits = Hits or 0
						...
						if A.Zone == "none" then
							if ActiveNameplates then
								for unitID in pairs(ActiveNameplates) do
									local nguid = UnitGUID(unitID)
									if nguid and (not _G.issecretvalue or not _G.issecretvalue(nguid)) and GUID == nguid then
										return unitID
									end
								end
							end
