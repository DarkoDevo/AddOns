...
MultiUnits.AddNameplate								= function(unitID)
	local canAttackOk, canAttack = pcall(UnitCanAttack, player, unitID)
	local isHostile = canAttackOk and canAttack
	if not canAttackOk then
		isHostile = true
	end
	if isHostile then
...
function A.MultiUnits.GetByRange(self, range, count)
	...
	if total == 0 then
		local ok, inRange = pcall(function() return A_Unit("target"):CanInterract(range) end)
		if ok and inRange and not A_Unit("target"):IsTotem() then
			total = total + 1
		end
	end
...
function A.MultiUnits.GetByRangeInCombat(self, range, count, upTTD)
	...
	if total == 0 then
		local ok, inRange = pcall(function() return A_Unit("target"):CanInterract(range) end)
		if ok and inRange and A_Unit("target"):CombatTime() > 0 and not A_Unit("target"):IsTotem() then
			total = total + 1
		end
	end
