...
local function IsDrinkingOrEating()
	...
	local auraData
	for i = 1, huge do
		auraData = UnwrapAuraData(UnitAura(player, i, "HELPFUL"))
...
local function PauseChecks()
	if not Player then Player = A.Player end
	...
	if 	(GetToggle(1, "CheckDeadOrGhost") and Unit(player):IsDead()) or
		(
			...
			(
				(Unit(target):IsDead() and (function() local ok,v = pcall(UnitIsFriend, player, target); if not ok then return true end; if _G.issecretvalue and _G.issecretvalue(v) then return true end; return not v end)() and (not A.IsInPvP or Unit(target):Class() ~= "HUNTER")) or
				(GetToggle(2, mouseover) and Unit(mouseover):IsDead() and (function() local ok,v = pcall(UnitIsFriend, player, mouseover); if not ok then return true end; if _G.issecretvalue and _G.issecretvalue(v) then return true end; return not v end)() and (not A.IsInPvP or Unit(mouseover):Class() ~= "HUNTER"))
			)
		)
...
function A.Rotation(icon)
	...
	if not A.IsInitialized or not APL or not Player then
		return A_Hide(icon)
	end
	...
	if meta <= 2 then
		if metatype == "function" then
			local ccOk, ccResult = pcall(metaobj, icon)
	...
	if meta == 5 then 
		...
		if metatype == "function" then
			local trinketOk, trinketResult = pcall(metaobj, icon)
			if trinketOk and trinketResult then
				return true
			end
		end
	...
	if #QueueData > 0 and QueueData[1] and ((not QueueData[1].MetaSlot and meta == 3) or QueueData[1].MetaSlot == meta) then
		SetMetaAlpha(meta, 1)
		if IsQueueReady(meta) then
			return QueueData[1]:Show(icon)
		end
		local qEntry = QueueData[1]
		local qTex = qEntry and qEntry.ID and TMW.GetSpellTexture(qEntry.ID)
		if qTex and type(qTex) == "number" then
			icon:SetInfo("state; texture", CONST.TMW_DEFAULT_STATE_SHOW, qTex)
		end
		return true
	end
	...
	local metaOk, metaResult = pcall(metaobj, icon)
	if metaOk and metaResult then
		if meta == 3 then SetMetaAlpha(meta, 1) end
		return true
	end
