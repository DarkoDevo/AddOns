...
local FallbackGetSpellInfo = _G.GetSpellInfo
local FallbackGetSpellName = C_Spell and C_Spell.GetSpellName
local function SafeGetActionSpellName(spellID)
        local getter = A_GetSpellInfo or A.GetSpellInfo
        if type(getter) == "function" then
                local ok, spellName = pcall(getter, spellID)
                if ok and spellName then
                        return spellName
                end
        end

        if type(FallbackGetSpellName) == "function" then
                local ok, spellName = pcall(FallbackGetSpellName, spellID)
                if ok and spellName then
                        return spellName
                end
        end

        if type(FallbackGetSpellInfo) == "function" then
                local ok, spellName = pcall(FallbackGetSpellInfo, spellID)
                if ok and spellName then
                        return spellName
                end
        end
			...
			["TMW:textlayout:1RkGJEN4L5o_"] = {
				{
					...
					["StringName"] = "ÐŸÑ€Ð¸Ð²ÑÐ·ÐºÐ°/Ð¯Ñ€Ð»Ñ‹Ðº",
					["Height"] = 1,
				}, -- [1]
				{
					["SkinAs"] = "Count",
	...
	local function UnitSpecsMapUpdate()
		...
		if z == "arena" and GetArenaOpponentSpec then
			...
			for i = 1, TeamCacheEnemy.MaxSize do 
				...
				if unit then
					local ok, n, s = pcall(UnitName, unit)
					if ok then
						name, server = n, s
					end
					if name and name ~= UNKNOWN then
						...
						pcall(function() UnitSpecsMap[name] = specID end)
					end
				end 
			end
		elseif z == "pvp" and GetBattlefieldScore then
			...
			local name, classToken, talentSpec, specID
			for i = 1, TeamCacheEnemy.MaxSize do
				local results = {pcall(GetBattlefieldScore, i)}
				if results[1] then
					name, classToken, talentSpec = results[2], results[10], results[17]
				end
				if name and classToken and classToken ~= "" then
					local roleMap = specNameToRole[classToken]
					if roleMap and talentSpec then
						specID = roleMap[talentSpec]
						pcall(function() UnitSpecsMap[name] = specID end)
					end
				end
			end
...
-- Midnight 12.0: COMBAT_LOG_EVENT_UNFILTERED is restricted/removed.
-- UNIT_SPELLCAST_SUCCEEDED covers all player cast tracking we need.
do
    ...
    if not module then
        ...
        -- Midnight fix: Removed COMBAT_LOG_EVENT_UNFILTERED handler entirely.
        -- CLEU is restricted in 11.x/12.0 and was causing ADDON_ACTION_FORBIDDEN taint
        -- via AceEvent's protected RegisterEvent call path.
        -- UNIT_SPELLCAST_SUCCEEDED already tracks all player casts we need.

        module:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED",
            function(_, unit, _, spellID)
                if unit == "player" and not blacklist[spellID] then
					local spellName                = SafeGetActionSpellName(spellID)
                    Env.LastPlayerCastName      = spellName and strlowerCache[spellName] or nil
