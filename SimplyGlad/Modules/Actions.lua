...
-- Remap
-------------------------------------------------------------------------------
local function SafeAOnGCD(...)
	if type(A.OnGCD) == "function" then
		return A.OnGCD(...)
	end
	if type(TMW.OnGCD) == "function" then
		return TMW.OnGCD(...)
	end
	return false
end
...
local function SafeAGetCurrentGCD()
	if type(A.GetCurrentGCD) == "function" then
...
local function SafeAGetGCD()
	if type(A.GetGCD) == "function" then
		return A.GetGCD()
	end
	return TMW.GCD or 0
end
...
local function SafeAGetPing()
	if type(A.GetPing) == "function" then
		return A.GetPing()
	end
	return 0
end
...
local function SafeAGetSpellInfo(...)
	if type(A.GetSpellInfo) == "function" then
		return A.GetSpellInfo(...)
	end
	if _G.C_Spell and type(_G.C_Spell.GetSpellInfo) == "function" then
		return _G.C_Spell.GetSpellInfo(...)
	end
...
local function SafeAGetSpellDescription(...)
	if type(A.GetSpellDescription) == "function" then
		return A.GetSpellDescription(...)
	end
	if _G.C_Spell and type(_G.C_Spell.GetSpellDescription) == "function" then
		return _G.C_Spell.GetSpellDescription(...)
	end
	if type(_G.GetSpellDescription) == "function" then
		return _G.GetSpellDescription(...)
	end
	return nullDescription
end

local A_OnGCD = SafeAOnGCD
local A_GetCurrentGCD = SafeAGetCurrentGCD
local A_GetGCD = SafeAGetGCD
local A_GetPing = SafeAGetPing
local A_GetSpellInfo = SafeAGetSpellInfo
local A_GetSpellDescription = SafeAGetSpellDescription

Listener:Add("ACTION_EVENT_ACTIONS", "ADDON_LOADED", function(addonName) 
	if addonName == CONST.ADDON_NAME then 
		A_OnGCD					= type(A.OnGCD) == "function" and A.OnGCD or SafeAOnGCD
		A_GetCurrentGCD			= type(A.GetCurrentGCD) == "function" and A.GetCurrentGCD or SafeAGetCurrentGCD
		A_GetGCD				= type(A.GetGCD) == "function" and A.GetGCD or SafeAGetGCD
		A_GetPing				= type(A.GetPing) == "function" and A.GetPing or SafeAGetPing
		A_GetSpellInfo			= type(A.GetSpellInfo) == "function" and A.GetSpellInfo or SafeAGetSpellInfo
		A_GetSpellDescription	= type(A.GetSpellDescription) == "function" and A.GetSpellDescription or SafeAGetSpellDescription
...
local                                         IsUsableSpell,                                          IsHelpfulSpell,                                          IsHarmfulSpell,                                             IsAttackSpell,                                          IsCurrentSpell,             IsSpellInSpellBook,             IsSpellKnown =
	  C_Spell and C_Spell.IsSpellUsable or _G.IsUsableSpell, C_Spell and C_Spell.IsSpellHelpful or _G.IsHelpfulSpell, C_Spell and C_Spell.IsSpellHarmful or _G.IsHarmfulSpell, C_Spell and C_Spell.IsAutoAttackSpell or _G.IsAttackSpell, C_Spell and C_Spell.IsCurrentSpell or _G.IsCurrentSpell, C_SpellBook.IsSpellInSpellBook, C_SpellBook.IsSpellKnown

local IsPlayerSpell 				= _G.IsPlayerSpell or function(spellID)
	return IsSpellKnown(spellID, SpellBookSpellBankPlayer)
end

local IsSpellKnownOrOverridesKnown 	= _G.IsSpellKnownOrOverridesKnown or function(spellID, isPet)
	return IsSpellInSpellBook(spellID, isPet and SpellBookSpellBankPet or SpellBookSpellBankPlayer, true)
end
...
local function getResolvedSpellID(spellID)
	if Compat and type(Compat.ResolveSpellID) == "function" then
		local ok, resolvedSpellID = pcall(Compat.ResolveSpellID, Compat, spellID)
...
local function safeSpellKnownOrOverridesKnown(spellID)
	if Compat and type(Compat.IsSpellKnownOrOverridesKnown) == "function" then
		local ok, result = pcall(Compat.IsSpellKnownOrOverridesKnown, Compat, spellID)
		if ok then
	...
	local resolvedSpellID = getResolvedSpellID(spellID)
	if resolvedSpellID ~= spellID then
		ok, result = pcall(IsSpellKnownOrOverridesKnown, resolvedSpellID)
		if ok and result then
			return true
		end
	end
...
local function safeSpellCooldownRemaining(spellID)
	if Compat and type(Compat.GetCooldownRemaining) == "function" then
		local ok, result = pcall(Compat.GetCooldownRemaining, Compat, spellID)
		if ok and type(result) == "number" then
			return result > 0 and result or 0
		end
	end

	local ok, result = pcall(CooldownDuration, spellID)
	if ok and type(result) == "number" then
		return result > 0 and result or 0
	end
...
local function safeSpellUsable(spellID, skipUsable)
	if skipUsable == true then
		return true
	end

	if type(skipUsable) == "number" then
		return Unit("player"):Power() >= skipUsable
	end

	if Compat and type(Compat.SafeIsSpellUsable) == "function" then
		local ok, result = pcall(Compat.SafeIsSpellUsable, Compat, spellID)
		if ok then
			return result and true or false
		end
	end
	...
	local resolvedSpellID = getResolvedSpellID(spellID)
	local ok, result = pcall(IsUsableSpell, resolvedSpellID)
	if ok then
...
local function getSpellRangeState(spellID, unitID)
	if Compat and type(Compat.SafeIsSpellInRange) == "function" then
		local ok, result = pcall(Compat.SafeIsSpellInRange, Compat, spellID, unitID)
		if ok then
			local normalized = normalizeCompatBoolean(result)
	...
	for i = 1, #idsToTry do
		local ok, result = pcall(IsSpellInRange, idsToTry[i], unitID)
		if ok then
			local normalized = normalizeCompatBoolean(result)
			if normalized ~= nil or result == nil then
				return normalized
...
local function canSpellPassRangeCheck(action, unitID)
	if action.skipRange or not unitID or not action:HasRange() then
		return true
	end

	local inRange = getSpellRangeState(action.ID, unitID)
	if inRange == nil or inRange == true then
		return true
	end

	return Pet:IsActive() and Pet:IsInRange(action.ID, unitID) or false
end

local function safeBlockCheck(action, methodName)
	local method = action and action[methodName]
...
function A.GetCurrentGCD()
	...
	local ok, result = pcall(CooldownDuration, "gcd")
	if ok and type(result) == "number" then
		return result > 0 and result or 0
	end
	return 0
...
function A:IsSpellInRange(unitID)
	...
	local inRange = getSpellRangeState(ID, unitID)
	if inRange ~= nil then
		return inRange or (Pet:IsActive() and Pet:IsInRange(ID, unitID)) or false
	end

	return Pet:IsActive() and Pet:IsInRange(ID, unitID) or false
end 
...
function A:IsUsable(extraCD, skipUsable)
	...
	if self.Type == "Spell" then 
		if not safeSpellUsable(self.ID, skipUsable) then
			return false
		end

		local cd = self:GetCooldown()
		local ok, ready = pcall(function()
			return cd <= A_GetPing() + CACHE_DEFAULT_TIMER + (self:IsRequiredGCD() and A_GetCurrentGCD() or 0) + (extraCD or 0)
		end)
		if not ok then
			if Compat and type(Compat.IsOnCooldown) == "function" then
				local cdok, onCD = pcall(Compat.IsOnCooldown, Compat, self.ID)
				if cdok and onCD == true then
					return false
				end
			end
			return safeSpellKnownOrOverridesKnown(self.ID) or (Pet:IsActive() and Pet:IsSpellKnown(self.ID))
		end

		return ready and (safeSpellKnownOrOverridesKnown(self.ID) or (Pet:IsActive() and Pet:IsSpellKnown(self.ID)))
	end 
...
function A:IsHarmful()
	-- @return boolean 
	if self.Type == "Spell" then 
		if Compat and type(Compat.SafeIsSpellHarmful) == "function" then
			local ok, result = pcall(Compat.SafeIsSpellHarmful, Compat, self.ID)
			if ok then
				return result and true or false
			end
		end
...
function A:IsHelpful()
	-- @return boolean 
	if self.Type == "Spell" then 
		if Compat and type(Compat.SafeIsSpellHelpful) == "function" then
			local ok, result = pcall(Compat.SafeIsSpellHelpful, Compat, self.ID)
			if ok then
				return result and true or false
			end
		end
...
function A:GetCooldown()
	...
	if self.Type == "Spell" then 
		if self.isStance then 
			...
			return safeSpellCooldownRemaining(self.ID)
		end 
	end 
...
function A:IsCastable(unitID, skipRange, skipShouldStop, isMsg, skipUsable)
	...
	if isMsg or ((skipShouldStop or not self.ShouldStop()) and not self:ShouldStopByGCD()) then 
		...
		--	( not self.isReplacement or self:IsExists(true) ) and 			
			( skipRange or not unitID or not self:HasRange() or canSpellPassRangeCheck(self, unitID) ) and 
...
function A:IsReady(unitID, skipRange, skipLua, skipShouldStop, skipUsable)
	-- @return boolean
	-- For [3-4, 6-8]
	return 	not safeBlockCheck(self, "IsBlocked") and 
				not safeBlockCheck(self, "IsBlockedByQueue") and 
