...
-- Midnight 12.0: Safe power check that handles secret values
local function SafeUnitPowerCheck(powerType, cost)
	local ok, result = pcall(function()
		return UnitPower("player", powerType) >= (cost or 0)
	end)
...
-- Secret Engine (Midnight 12.0) — must be loaded before Engines/Unit.lua, Player.lua, Core.lua
Action.SecretEngine													= _G.ActionSecretEngine
...
local function DispelPurgeEnrageRemap()
	...
	if Action.SecretEngine and Action.SecretEngine.RegisterDispelSpell then
		local dispelCategories = { "Poison", "Disease", "Curse", "Magic" }
		for _, mode_v in pairs(gActionDB[5]) do
			for _, cat in ipairs(dispelCategories) do
				if mode_v[cat] then
					for spellID in pairs(mode_v[cat]) do
						Action.SecretEngine:RegisterDispelSpell(spellID, cat)
					end
				end
			end
		end
		for _, specTable in pairs(pActionDB[5]) do
			for _, Mode in ipairs({"PvE", "PvP"}) do
				if specTable[Mode] then
					for Category, catTable in pairs(specTable[Mode]) do
						if type(catTable) == "table" and catTable[GameLocale] then
							for _, entry in pairs(catTable[GameLocale]) do
								if type(entry) == "table" and entry.ID and entry.Enabled then
									local dt = nil
									for _, c in ipairs(dispelCategories) do
										if Category == c or Category:find(c) then dt = c; break end
									end
									if not dt and (Category == "Dispel" or Category == "MagicMovement") then dt = "Magic" end
									if dt then
										Action.SecretEngine:RegisterDispelSpell(entry.ID, dt)
									end
								end
							end
						end
	...
	local UnitAuras = {
        ...
        [ActionConst.DEMONHUNTER_DEVOURER] = {
            PvE = {
				BlackList = {
					ActionDataAuras.PvE.BlackList,
				},
...
local Queue; Queue 				= {
	...
	UNIT_SPELLCAST_SUCCEEDED 	= function(...)
		...
		if (source == "player" or source == "pet") and ActionDataQ[1] and ActionDataQ[1].Type == "Spell" then
			local queuedID = ActionDataQ[1].ID
			local matched = queuedID and spellID and queuedID == spellID
			if not matched then
				local SE = _G.ActionSecretEngine
				if SE and SE.GetBaseSpellID then
					local baseQueued = SE.GetBaseSpellID(queuedID)
					local baseSpell = SE.GetBaseSpellID(spellID)
					matched = (baseQueued and baseQueued == spellID) or (baseSpell and baseSpell == queuedID)
				end
			end
			if not matched then
				local nok, nameMatch = pcall(function() return A_GetSpellInfo(spellID) == ActionDataQ[1]:Info() end)
				matched = nok and nameMatch
			end
			if matched then
				ActionDataQ[1].CastConfirm = nil
				getmetatable(ActionDataQ[1]).__index:SetQueue({ Silence = true, CastConfirmed = true })
			end
		end
	end,
...
function Action:QueueValidCheck()
	...
	local castNameMatch = false
	if isCastingName then
		local cok, cmatch = pcall(function() return isCastingName == self:Info() end)
		castNameMatch = cok and cmatch
	end
	if (not isCastingName or not castNameMatch) and (not isChannel or Queue.IsInterruptAbleChannel[castID]) then
		if self.Type == "SwapEquip" or self.isStance then
			return true
		else
			-- pcall HasRange because SpellHasRange can return secret values in Midnight
			local hrOk, hasRange = pcall(function() return self:HasRange() and true or false end)
			if not hrOk then hasRange = false end
			...
			-- Helpful spells targeting an enemy should fall back to "player" (e.g. Astral Shift is a self-buff)
			if not isHarm and unitID == "target" and not UnitIsFriend("player", "target") then
				unitID = "player"
			end
			self.UnitID		= unitID
...
function Action.IsQueueReady(meta)
    ...
    if index > 0 and Queue.IsThisMeta(meta) then 		
		...
		-- Cancel 
		if self.Auto and not self.CastConfirm and self.Start and TMW.time - self.Start > (ActionData.QueueAutoResetTimer or 10) then
			Queue.OnEventToReset()
			return false 
		end 	
		
		if not Queue.IsTypeValid[self.Type or ""] then
			local qlok, qlval = pcall(function() return self:Link() end)
			local qSafeLink = (qlok and type(qlval) == "string") and qlval or ("[" .. tostring(self.ID) .. "]")
			A_Print(L["DEBUG"] .. qSafeLink .. " " .. L["ISNOTFOUND"])
		...
		if self.Type == "SwapEquip" then
			return 	not A_Player:IsSwapLocked()
					and (not self.PowerCustom or SafeUnitPowerCheck(self.PowerType, self.PowerCost))
			...
			local usableOk = self:IsUsable(self.ExtraCD)
			if not usableOk and self.Type == "Spell" then
				local cd = self:GetCooldown()
				local cdOk, cdReady = pcall(function() return cd <= A_GetPing() + 0.01 + (self:IsRequiredGCD() and A_GetCurrentGCD() or 0) + (self.ExtraCD or 0) end)
				if cdOk and cdReady then usableOk = true end
			end
...
function Action:IsBlockedByQueue()
	...
	return 	not self.QueueForbidden  
			...
			and ( not ActionDataQ[1].PowerCost or not SafeUnitPowerCheck(self.PowerType, ActionDataQ[1].PowerCost) )
...
function Action:SetQueue(args) 
	...
	-- Safe link for prints (GetSpellLink returns secret in Midnight)
	local safeLink
	do
		local lok, lval = pcall(function() return self:Link() end)
		safeLink = (lok and type(lval) == "string") and lval or ("[" .. tostring(self.ID) .. "]")
	end

	-- Check validance
	if not self.Queued then
		local existsOk, exists = pcall(function() return self:IsExists() end)
		if (not existsOk or not exists) or self:IsBlockedBySpellLevel() then
			A_Print(L["DEBUG"] .. safeLink .. " " .. L["ISNOTFOUND"])
			return
		end
	end
...
--		return
	...
	if args.Value ~= nil and self.Queued == args.Value then
		if not args.Silence then
			A_Print(L["DEBUG"] .. safeLink .. " " .. L["TAB"][3]["ISQUEUEDALREADY"] .. printKey)
		end
		return
	end
    ...
    if not args.Silence then
		if self.Queued then
			A_Print(L["TAB"][3]["QUEUED"] .. safeLink .. L["TAB"][3]["QUEUEPRIORITY"] .. priority .. ". " .. L["TAB"][3]["KEYTOTAL"] .. #ActionDataQ + 1 .. "]")
		else
			A_Print(L["TAB"][3]["QUEUEREMOVED"] .. safeLink .. printKey)
		end
    end
	...
	if not self.Queued then
		for i = #ActionDataQ, 1, -1 do
			if ActionDataQ[i]:GetTableKeyIdentify() == Identify then
				if ActionDataQ[i].CastConfirm and not args.CastConfirmed and not args.Priority then
					self.Queued = true
					return
				end
	...
	if args.CastConfirm then
		ActionDataQ[priority].CastConfirm = true
	end
...
function Action.MacroQueue(key, args)
	...
	if object.Type == "Spell" then
		args.CastConfirm = true
	end
		...
		__index = function(t, category)
			t[category] = setmetatable({}, { 
				__call = function(this, spellName, castStartTime, castEndTime, countGCD)
					...
					local eqOk, neq = pcall(function() return thisCast.lastEndTime ~= castEndTime or thisCast.countGCD ~= countGCD end)
					if (not eqOk or neq) then
						local ftOk, castFullTime = pcall(function() return castEndTime - castStartTime end)
						if not ftOk then return 0 end
						local min, max				
...
function Action.InterruptIsValid(unitID, toggle, ignoreToggle, countGCD)
	...
	if ignoreToggle or A_InterruptIsON(toggle) then 	
		...
		if type(spellName) == "string" then
			...
			local cok, cst, cet, cdt, crt = pcall(function()
				local s = castStartTime / 1000
				local e = castEndTime / 1000
				return s, e, TMW.time - s, e - TMW.time
			end)
			if not cok then
				return false, false, false, notInterruptable, 0, 0
			end
			castStartTime = cst
			castEndTime = cet
			castDoneTime = cdt
			...
			if not MainAuto and not MouseAuto then
				local iOk, iVal = pcall(function() return pActionDB[4][category][GameLocale][spellName] end)
				Interrupt = iOk and iVal
			...
			local blackListedKick, blackListedCC, blackListedRacial
			pcall(function() blackListedKick, blackListedCC, blackListedRacial = A_InterruptIsBlackListed(unitID, spellName) end)
			...
			local mdtOk, mustDoneTime = pcall(function() return Interrupts.CastMustDoneTimeByToggle[category](spellName, castStartTime, castEndTime, countGCD or notInterruptable) end)
			if not mdtOk then return false, false, false, notInterruptable, castRemainsTime, castDoneTime end
			...
			if toggle == "Mouse" then 				
				if MouseAuto then 
					...
					if category == "MousePvP" then 
						local ieOk, iePvP, ieHeal = pcall(function() return A_InterruptEnabled("PvP", spellName), A_InterruptEnabled("Heal", spellName) end)
					if not A_Unit(unitID):IsPlayer() or (ieOk and not iePvP and not ieHeal) or not ieOk then
							return false, false, false, notInterruptable, castRemainsTime, castDoneTime
						end 
					end 
				end 
		...
		if Aura and not A_AuraIsBlackListed(unitID) then
			...
			for i = 1, huge do
				...
				if auraData then
					...
					if aok and ares then return true end
				else
					break
				end
			end
		end
		if Toggle == "UseDispel" and Action.SecretEngine and Action.SecretEngine.UnitNeedsDispel then
			local dispelType = (Category == "Magic" or Category == "Curse" or Category == "Poison" or Category == "Disease") and Category or nil
			if Action.SecretEngine:UnitNeedsDispel(unitID, dispelType) then
				return true
			end
		end
	end
...
function Action.RegisterDispelSpell(spellID, dispelType, opts)
	if not spellID or not dispelType then return end
	local mode = (opts and opts.mode) or "PvE"
	if not gActionDB[5][mode] then gActionDB[5][mode] = {} end
	if not gActionDB[5][mode][dispelType] then gActionDB[5][mode][dispelType] = {} end
	if not gActionDB[5][mode][dispelType][spellID] then
		gActionDB[5][mode][dispelType][spellID] = opts or { byID = true }
	end
	if Action.SecretEngine and Action.SecretEngine.RegisterDispelSpell then
		Action.SecretEngine:RegisterDispelSpell(spellID, dispelType)
	end
end

function Action.RegisterDispelSpells(mappings)
	for i = 1, #mappings do
		local m = mappings[i]
		Action.RegisterDispelSpell(m[1], m[2], m[3])
	end
end
	...
	OnEvent 				= function(self)
		...
		if Cursor.Initialized then 
			...
			if UseLeft or UseRight then 
				...
				local objSec = _G.issecretvalue and ObjectName and _G.issecretvalue(ObjectName)
				if ObjectName and not objSec then
					-- UnitName
		...
		if tabName == 3 then 
			...
			SetQueue:SetScript("OnClick", function(self, button, down)
				...
				if not index then 
					...
					local data = ScrollTable:GetRow(index)
					local dlok, dlval = pcall(function() return data:Link() end)
					local dSafeLink = (dlok and type(dlval) == "string") and dlval or ("[" .. tostring(data.ID) .. "]")
					if data.QueueForbidden or ((data.Type == "Trinket" or data.Type == "Item") and not data:GetItemSpell()) then
						Action.Print(L["DEBUG"] .. dSafeLink .. " " .. L["TAB"][3]["ISFORBIDDENFORQUEUE"])
...
local function DisableAssistedHighlight()
	if GetCVar("assistedCombatHighlight") == "1" then
		SetCVar("assistedCombatHighlight", "0")
		print("|cFFFF6600[Action]|r Assisted Combat Highlight has been disabled — it conflicts with this addon.")
	end
end

local function CheckEnemyNameplates()
	if GetCVar("nameplateShowEnemies") ~= "1" then
		print("|cFFFF6600[Action]|r WARNING: Enemy Nameplates are disabled! AoE detection will not work correctly. Enable them in Options > Names > Enemy Units.")
	end
end

local settingsCheckFrame = CreateFrame("Frame")
settingsCheckFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
settingsCheckFrame:RegisterEvent("CVAR_UPDATE")
settingsCheckFrame:SetScript("OnEvent", function(_, event, cvar)
	if event == "PLAYER_ENTERING_WORLD" then
		DisableAssistedHighlight()
		CheckEnemyNameplates()
	elseif event == "CVAR_UPDATE" then
		if cvar == "assistedCombatHighlight" then
			C_Timer.After(0, DisableAssistedHighlight)
		elseif cvar == "nameplateShowEnemies" then
			C_Timer.After(0, CheckEnemyNameplates)
		end
	end
end)
...
local function OnInitialize()	
	...
	if not Action.IsInitializedModifiedTMW and TMW then
		...
		function TMW:OnUpdate()
			...
			local rok, rerr = pcall(coroutine.resume, Coroutine)
			if not rok then
				if not Action._dbgCoro or (_G.GetTime() - Action._dbgCoro > 5) then
					Action._dbgCoro = _G.GetTime()
					_G.print("|cFFFF0000[TMW Coroutine]|r " .. tostring(rerr))
				end
				Coroutine = nil
			end
		end
