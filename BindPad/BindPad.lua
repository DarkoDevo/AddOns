...
-- Register BindPad frames to be controlled together with other panels in the standard UI.
-- Newer client builds may protect UIPanelWindows; fail safely if it is forbidden/restricted.
local function BindPad_SafeRegisterUIPanelWindow(frameName, info)
    local upw = _G.UIPanelWindows
    if type(upw) ~= "table" then
        return
    end
    pcall(function()
        upw[frameName] = info
...
BindPad_SafeRegisterUIPanelWindow("BindPadFrame", { area = "left", pushable = 8, whileDead = 1 })
BindPad_SafeRegisterUIPanelWindow("BindPadMacroFrame", { area = "left", pushable = 9, whileDead = 1 })
...
function BindPadBindFrame_OnKeyDown(self, keyOrButton)
    ...
    if keyPressed == "LSHIFT" or keyPressed == "RSHIFT" or keyPressed == "LCTRL" or keyPressed == "RCTRL" or keyPressed == "LALT" or keyPressed == "RALT" then
        return
    end
...
function BindPadProfileTab_OnEnter(self, motion)
    ...
    elseif spec3 ~= nil then
        GameTooltip:AddLine(
            format(BINDPAD_TOOLTIP_PROFILE_CURRENTLY3, BindPadCore.GetTalentSpec(spec1), BindPadCore.GetTalentSpec(spec2), BindPadCore.GetTalentSpec(spec3)),
            0.8,
            0.8,
            1.0
        )
    elseif spec2 ~= nil then
        GameTooltip:AddLine(format(BINDPAD_TOOLTIP_PROFILE_CURRENTLY2, BindPadCore.GetTalentSpec(spec1), BindPadCore.GetTalentSpec(spec2)), 0.8, 0.8, 1.0)
